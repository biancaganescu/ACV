...
from toolemu.tools import get_manager_cls_from_tool, get_toolkits_by_names
...
class SimulatorInputModel(BaseModel):
    ...
    sensitive_context: Optional[str]
    essential_context: Optional[str]
    context_list: Optional[Dict]
...
def extract_json_dict(text):
    """
    Extracts the first JSON dictionary found in a string and converts it to a dict.
    """
    try:
        ...
        if match:
            ...
            # Standard json.loads requires double quotes " "
    ...
    def _take_next_step(
        ...
        for agent_action in actions:
            ...
            if agent_action.tool in name_to_tool_map:
                ...
                toolkit_name = get_manager_cls_from_tool(tool)
                ...
                try:
                    sensitive_context = inputs["toolkits_data"][toolkit_name.__name__]["sensitive"]
                    essential_context = inputs["toolkits_data"][toolkit_name.__name__]["essential"]
                except KeyError:
                    sensitive_context = f"No sensitive context for {toolkit_name.__name__}"
                    essential_context = f"No essential context for {toolkit_name.__name__}"
                ...
                # pattern = r"Sensitive Observation:\s*(.*?)\s*Essential Observation:\s*(.*)"
                # match = re.search(pattern, observation[0], re.DOTALL)
                # sensitive_obs_str = match.group(1).strip()
                # essential_obs_str = match.group(2).strip()
                # sensitive_obs = extract_json_dict(sensitive_obs_str)
                # essential_obs = extract_json_dict(essential_obs_str)
                # if "search"in agent_action.tool.lower() or "read" in agent_action.tool.lower():
                #     merged_obs = sensitive_obs.copy()
                #     merged_obs["results"].extend(essential_obs["results"])
                #     observation = json.dumps(merged_obs, indent=2)
                # elif "send" in agent_action.tool.lower() or "create" in agent_action.tool.lower() or "add" in agent_action.tool.lower():
                #     merged_obs = sensitive_obs.copy()
                #     merged_obs.update(essential_obs)
                #     observation = json.dumps(merged_obs, indent=2)

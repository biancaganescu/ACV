...
AGENT_TYPES = ["naive", "privacy_enhanced", "ci_cot"]
...
class ZeroShotAgentWithToolkit(ZeroShotAgent):
    ...
    def create_prompt(
        ...
        system_info = eval(f"AGENT_{prompt_type.upper()}_SYSTEM_INFO")
        ...
        if use_chat_format:
            ...
            if input_variables is None:
                input_variables = ["user_instruction", "agent_scratchpad"]
    ...
    def _get_model_name(self) -> str:
        """Get the model name, compatible with both deployment_name and model_name."""
        return getattr(self.llm_chain.llm, 'deployment_name', None) or getattr(self.llm_chain.llm, 'model_name', '')
    ...
    def _get_next_action(self, full_inputs: Dict[str, str]) -> AgentAction:
        ...
        if "gpt-5" in self._get_model_name() or "o3" in self._get_model_name():
            for stop_seq in self._stop:

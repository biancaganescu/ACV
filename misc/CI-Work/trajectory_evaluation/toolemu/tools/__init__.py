from langchain.tools import BaseTool

from .core_virtual_tools import *
from .register import get_toolkit_dict, toolkits_factory
from .tool_interface import BaseToolkit, FunctionToolkit


def get_toolkits_by_names(names: List[str]) -> List[FunctionToolkit]:
    toolkits = []
    for name in names:
        toolkit = toolkits_factory(name)
        if toolkit:
            toolkits.append(toolkit())
        else:
            print(f"Warning: toolkit {name} not found")
    return toolkits


def get_tool_class_by_name(toolkits: List[FunctionToolkit], name: str) -> BaseTool:
    for toolkit in toolkits:
        try:
            return toolkit[name]
        except ValueError:
            pass
    raise ValueError(f"Tool {name} does not exist in these toolkits")

def get_manager_cls_from_tool(tool_instance):
    """Given a tool instance, return the corresponding Toolkit class."""
    toolkit_dict = get_toolkit_dict()
    
    tool_cls = type(tool_instance)
    
    for name, toolkit_cls in toolkit_dict.items():
        if hasattr(toolkit_cls, 'tool_classes'):
            if tool_cls in toolkit_cls.tool_classes:
                return toolkit_cls
                
    return None
    ...
    returns: List[ArgReturn] = [
        {
            "name": "events",
            "type": "array",
            "description": "An array of objects, each containing the details of an event, including id, event_name, content, start_time, end_time, location, and attendees.",
        }
    ]
    ...
    returns: List[ArgReturn] = [
        {
            "name": "emails",
            "type": "array",
            "description": "An array of at most 'limit' emails that match the search criteria, each containing the 'id', 'subject', 'from' (The email address the email was sent from), 'to' (A string of email addresses that the email was sent to, separated by commas), 'cc' (A string of email addresses that the email was cc'ed to, separated by commas), 'timestamp' (in the format 'YYYY-MM-DD HH:mm'), 'body' (The body of the email).",
        }
    ]
    exceptions: List[ArgException] = [
        {
    ...
    returns: List[ArgReturn] = [
        {
            "name": "messages",
            "type": "array",
            "description": "The search results, each object includes 'id', (the id of the found message)  'content' (string, the content of the message), 'timestamp' (string, the timestamp of the message in ISO 8601 format), 'in' (string, the channel name or user name), 'from' (string, the user name of the sender), and 'file_id' (string, the unique identifier of the file if the message is a file).",
        }
    ]
    exceptions: List[ArgException] = [
        {

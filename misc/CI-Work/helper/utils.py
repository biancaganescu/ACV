...
def setup_prompt_environment(template_name: str):
    from jinja2 import Environment, FileSystemLoader, StrictUndefined

    env = Environment(
        loader=FileSystemLoader(template_name),
...
def load_json_answer(answer):
    ...
    # Attempt to extract JSON from generic code blocks
    code_match = re.search(r"```\s*(\{.*?\})\s*```", answer, re.DOTALL)
    if code_match:
